...
- **로직**: Yahoo Finance `chart` API(월봉, 기본 정보 + CAGR/MDD/변동성 계산)와 `quoteSummary` API(배당률/운용보수/베타 등, crumb 인증 필요)를 병렬 조회 후 병합. 국내 6자리 티커는 `.KS` 심볼로 변환. 응답에 프론트 공통 구간 재계산용 `chartData: { t, c }[]`(월별 타임스탬프+종가) 포함.
