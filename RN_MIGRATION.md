...
React Native 전면 전환 계획. **코드 작업 착수 전 단계 문서** — 실제 이식은 4절(토스 트랙)과 9절(미해결 질문)이
정리된 뒤 시작한다.
...
2026-07-30 구글 플레이 프로덕션 액세스 신청이 거절됐다. 이후 네이티브 전환을 결정했고, 이 문서는 그 전환 계획이다.

**단, 기록해 둘 것 — 거절 메일과 RN 전환은 별개 사안이다.**

거절 메일 제목은 "프로덕션에 액세스하려면 추가 테스트 필요"이고, 사유는 두 가지였다.

1. 비공개 테스트 중 테스터가 앱에 참여(= 실사용)하지 않음
2. 사용자 의견을 수집하고 이를 반영한 앱 업데이트를 올리지 않음

둘 다 2023-11-13 이후 개설된 개인 개발자 계정에 적용되는 **비공개 테스트 요건**(테스터 12명 × 14일 연속 옵트인
+ 실사용 + 피드백 반영 업데이트)에 대한 것이고, 앱이 어떤 기술로 만들어졌는지는 이 단계에서 심사 대상이 아니다.
TWA는 오히려 구글이 PWA 배포용으로 공식 권장하는 방식이며, 2026년에 강화된 정책 4.3(최소 기능) 자동 심사의
표적은 서비스 워커도 없는 raw WebView 래퍼다. FirePath는 `vite-plugin-pwa` 기반의 정식 PWA다.

→ **RN으로 전환해도 위 두 사유는 그대로 남는다.** 재신청하려면 어차피 14일 비공개 테스트를 다시 채워야 하고,
그 기간에 테스터 실사용과 피드백 반영 업데이트 이력을 만들어야 한다. 이 작업은 RN 전환과 **병행해서 별도로**
진행해야 한다. 전환 완료를 기다렸다가 시작하면 그만큼 출시가 밀린다.
...
## 4. 토스 미니앱 트랙 — ⚠️ 착수 전 결정 필요

**이게 이번 전환에서 가장 큰 미결 변수다.**
...
> 전환 범위가 14% 줄어든다. (4절에서 (a)안을 택해 웹이 어차피 유지된다면 특히.)
...
| 리스크 | 영향 | 대응 |
| --- | --- | --- |
| **토스 트랙 미결** (4절) | 코드베이스 1개 vs 2개 — 장기 유지비가 갈림 | **착수 전 확인.** 최우선 |
...
1. **앱인토스 RN 프레임워크로 미니앱 트랙을 옮길 수 있는가?** (4절) — 답에 따라 전체 구조가 갈린다.
2. **관리자 모듈 10개를 RN으로 옮길 것인가, 웹에 남길 것인가?** — 전환 범위 14% 차이.
3. **PWA/웹 버전을 계속 유지하는가?** — 유지한다면 웹 기능 동결 정책이 필요.
4. UI는 자체 프리미티브(권장)인가 React Native Paper인가? (3-2절) — 일정 우선이면 뒤집을 수 있다.
5. iOS도 같이 낼 것인가? — RN 전환의 최대 이점이지만 범위가 또 늘어난다.
