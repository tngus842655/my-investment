...
- ✅ 사용자 단계 1 (SQL 실행) — 2026-07-13 실행 완료, 검증 쿼리 둘 다 0 확인.
- ✅ Claude 단계 2 (표시·집계 환산 + 통화 포맷) — 2026-07-13 완료.
  `src/utils/budgetMoney.ts` 신설: `loadRatesToBase`(행 통화→기준통화 환율 맵, 기준통화와 같으면 조회
  안 함 — 전 행 동일 통화인 대부분 사용자는 환율 API 호출 0회), `toBaseAmount`,
  `formatBudgetAmount`(단건: 기록된 통화 그대로, ko+KRW는 기존 "12,000원"과 문자열 동일),
  `formatBudgetSumBare`(접미사 없던 집계 자리: ko+KRW는 기존 "12,000" 그대로, 그 외 "$1,234").
  캘린더(요약 3종/캘린더 셀/선택 날짜/일일 그룹/월별 12행 — reduce 10곳)와 통계(수입·지출 토글/도넛
  중앙·hover/범례/카테고리 세그먼트)는 행별 통화 환산 후 합산. 검색/즐겨찾기/내역추가 칩의 단건 금액은
  기록된 통화 그대로 표시. 엑셀 미리보기 금액은 기준통화로 표시. 내역추가 금액 상한을 통화별로 분리
