...
결제수단(현금/카드 등). 카테고리와 동일한 방식 — 사용자가 직접 추가·수정·삭제 가능. 결제수단이 0개면 결제수단 관리 화면(`BudgetPaymentMethodView.vue`)에 "기본 결제수단 추가" 버튼이 노출되어, 누르면 기본 세트(현금/카드)가 시딩된다 (`src/utils/budgetDefaultPaymentMethods.ts`) — 삭제 직후 자동으로 되살아나면 혼란스러워서 자동 시딩 대신 수동 버튼 방식으로 변경.
