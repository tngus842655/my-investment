...
| `android/.../MainActivity.java` | 시스템 글자 크기 무시 (`setTextZoom(100)`) — 9절 참고 |
...
- [ ] **시스템 글자 크기 무시** — 기기 설정에서 글자 크기를 최대로 올린 뒤 앱을 켰을 때, 글자 크기가
      그대로인가 (앱 설정 > 화면 설정의 조절은 여전히 먹는가 — 9절 참고)
...
기기 설정(설정 > 디스플레이 > 글자 크기)을 크게 해두고 앱에 들어오면 글자가 과하게 커져 화면이
깨지던 문제를 막는다. `MainActivity.onCreate`에서 웹뷰 배율을 100%로 고정한다.
...
안드로이드 웹뷰는 기기 설정의 글자 크기를 `textZoom`에 **자동으로** 반영한다. 그런데 앱에도 자체
글자 크기 조절(설정 > 화면 설정, `useFontScale`, 75~150%)이 있어서 **두 배율이 곱해진다.**
시스템 150% × 앱 150% = 225%가 되니 레이아웃이 버티지 못한다.

100으로 고정하면 앱 안의 조절만 적용되고, 사용자가 글자를 키우고 싶으면 앱 설정에서 하면 된다.

### 알아둘 것

- **접근성 트레이드오프가 있다.** 안드로이드 시스템 글자 크기는 기기에 따라 200%까지 올라가지만
  앱 자체 조절은 150%가 상한이다. 시스템을 200%로 쓰던 사용자는 최대 글자 크기가 작아진다.
  더 필요하면 `src/design/index.ts`의 `FONT_SCALE_MAX`를 올릴 것.
- **웹·PWA에는 적용되지 않는다.** 네이티브 코드라 앱에서만 동작한다. 안드로이드 크롬으로 접속하면
  시스템 글자 크기가 그대로 먹는다. 브라우저에서까지 사용자 설정을 덮는 건 과하다고 보고 두었다.
- **"디스플레이 크기"(화면 배율)는 별개 설정이다.** 그건 글자만이 아니라 밀도(dp) 전체를 바꾸는
  것이라 `setTextZoom`으로 막히지 않고, 막아서도 안 된다.
- 시스템 글자 크기를 바꾸면 액티비티가 재생성된다(`AndroidManifest.xml`의 `configChanges`에
  `fontScale`이 없다). 그때 `onCreate`가 다시 돌며 재고정하므로 별도 처리가 필요 없다.
