...
const notes = [
  {
    ...
    description: [],
    items: [
      '가계부 내역추가 팝업 개편 — 날짜/카테고리/금액 입력을 화면 하단 고정 패널(달력·그리드·커스텀 숫자 키패드)로 전환, 기기 화면 크기에 맞춰 자동 축소되어 잘리거나 스크롤 없이 표시, 입력 순서를 날짜→카테고리→금액→내용으로 통일',
      '가계부 카테고리/결제수단 기본값 자동 재시딩을 수동 "기본 추가" 버튼 방식으로 변경 — 삭제하자마자 자동으로 되살아나 혼란스럽던 문제 해결, 카테고리는 지출/수입 타입별로 독립 관리',
      '가계부 금액 입력 버그 수정 — 앞자리 0이 그대로 남던 문제(예: 0,008,566), 유효성 메시지가 나타났다 사라지며 레이아웃이 밀리던 문제',
      '가계부 캘린더 버그 수정 — 내역 수정 후 스와이프 카드가 열린 채로 남던 문제, 날짜 선택 캘린더가 엉뚱한 달(1월)로 열리던 문제',
      '가계부 엑셀 가져오기 개선 — "엑셀 양식 다운로드" 기능 신규 추가, 열 순서 고정 방식 대신 제목 행 텍스트로 열을 인식하도록 변경(순서가 바뀌어도 정상 인식, 필수 열 누락 시 에러 안내), 화면 안내 문구 가독성 개선 및 PC 사용 권장 안내 추가',
      '가계부 상단에 검색 아이콘 추가',
      '가계부 데이터 초기화·회원탈퇴 비밀번호 확인 다이얼로그의 크롬 콘솔 경고(비밀번호 자동완성 오작동 포함) 수정',
